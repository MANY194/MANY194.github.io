...
function grid() {
	...
	//NIVEL DIFICULTAD
	let modal = document.getElementById("jugar");
...
function generarCartas(jugar, numImg, tematica) {
	cronometrar();
	cargarNumPartidas();

	let parentElement = document.getElementById("wrapper");
	let numElements = jugar * jugar;
	...
	parentElement.style.setProperty('--rowNum', jugar);
	parentElement.style.setProperty('--colNum', jugar);
}
...
function carta(contenedor, img, numImg) {
	...
	imgReverso.setAttribute("src", "reverso.png");
